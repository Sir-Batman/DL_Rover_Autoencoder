...
int main(){
	...
	std::string fileDirString = fileDir;

	std::string poi_laser_fname = fileDirString + "/poi_laser.csv";
	std::string rov_laser_fname = fileDirString + "/rov_laser.csv";
	...
	// 		if (staticOrRandom == 0)
	// 			//why do we do trainDomain.InitialiseEpoch() for both when staticOrRandom==0 and== 1, seems like they should be different things.
	...
	// 	trainDomain.OutputLaserData(poi_laser_fname, rov_laser_fname);
	// 	trainDomain.WriteLaserData();
	// }
